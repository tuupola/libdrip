...
TEST test_filter_chain_unordered_and_extra(void) {
    ...
    ASSERT_MEM_EQ(&links[0], &out[2], sizeof(drip_link_t));
    ASSERT_MEM_EQ(&root, &out[3], sizeof(drip_link_t));
    PASS();
}
...
TEST test_filter_chain_self_signed_only(void) {
    ...
    int rc = drip_link_filter_chain(
        &root, 1, drip_link_get_child_det(&root), out, 4, &out_count
    );
    ASSERT_EQ(DRIP_SUCCESS, rc);
    ASSERT_EQ(1, out_count);
    ASSERT_MEM_EQ(&root, &out[0], sizeof(drip_link_t));
    PASS();
}
...
SUITE(link_suite) {
    ...
    RUN_TEST(test_verify_chain_last_hop_unixtime_expired);
